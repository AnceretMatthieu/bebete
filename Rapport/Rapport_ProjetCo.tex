...
Enfin, au sein même de l'équipe, nous avons essayé d'organiser des points le plus régulièrement possible. Ces points permettaient de partager les difficultés rencontrées, de connaitre l'avancement des membres, de savoir ce qu'il restait à faire... Nous faisions un point par semaine, de préférence en début, qui durait en moyenne 10 à 15 minutes. 
	...
	\section{"Feedback" sur le projet}
	<!-- Une citation par membre de l'équipe (environ 10-15 lignes) revenant sur le déroulement du projet -->
	...
	\begin{quotation}
	J?ai eu des difficultés quand à l?utilisation de Qt sous Linux car nous travaillons sous différents systèmes d?exploitation et ma version n?avait pas les paquets à jour. Je suis donc passé sous Windows 7 pour continuer le projet.
	Ma seconde difficulté fut l?utilisation de Git. J?ai effectivement eu beaucoup de mal à comprendre le fonctionnement des dépôts ainsi que de l?utilisation de Git. J?ai eu différents problèmes à plusieurs moments du projet.
	\end{quotation}
...
Globalement, le projet est utilisable en l'état actuel. Il reste malgré tout plusieurs éléments à terminer ou à compléter : 
\begin{itemize}
	\item Re-travailler l'ergonomie de l'application PC, notamment pour la partie faisant le lien entre une réponse et sa question associée. 
	\item Re-organiser le dossier contenant les différents médias (par exemple, un dossier par morpho-espèce ou un dossier par branche de l'arbre)
	\item 
	
	\item Affichage de tous les résultats (pouvoir consulter les stats/résultats directement sur la tablette)
	\item Pouvoir changer de fichier XML à la volée (via les paramètres)
	\item Automatiser l'application mobile : via un serveur par exemple, qui viendrait automatiquement vérifier si une nouvelle version du fichier XML existe. Cette nouvelle version serait mise à disposition via l'application PC. 
\end{itemize}
