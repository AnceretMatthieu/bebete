...
type de l'insecte et de décider s'il est nuisible ou non. L'application va poser un certain nombre
...
\chapter{Analyses complémentaires}
Nous allons présenter ici les modélisations et diagrammes UML réalisés a posteriori du cahier de spécifications. En effet, lors de la rédaction du cahier de spécification, nous n'avions pas encore modélisé la partie "intelligence" des applications.  
	...
	Avant de développer l'application PC, nous avons pris soin de modéliser une structure fiable et efficace à même de réaliser toutes les fonctionnalités demandées et de pouvoir évoluer de façon simple. La figure \ref{diagramme classes appli PC} explique l'architecture de notre application PC. 
	
\begin{figure}[!h]
	\centering	
	\includegraphics[width=1\textwidth]{images/diag_classes_app_PC.jpg}
...
Maintenant que nous avons identifié les classes que nous allons utiliser, il faut les remplir avec des attributs. Outre les informations propres à chacune des classes, par exemple le texte associé a une question, il faut penser à garder la notion d'arborescence (c'est-à-dire la notion de parenté). Pour cela, chaque classe possède un attribut qui pointe vers son père et une liste contenant des pointeurs vers ses fils. C'est ici qu'interviennent nos classes ListeXXXX. Elles sont utile pour représenter la notion de parenté, mais elles nous permettent aussi de ne pas avoir à re-lire notre fichier XML à chaque recherche des fils d'un objet. Le seul point négatif de cette méthode est qu'elle nous oblige à charger en mémoire la totalité de l'arbre au lancement de l'application. Mais cela nous permet aussi, en cas de modification d'éléments, de différer la sauvegarde (tout est en mémoire). De ce fait, il y a plus de souplesse dans la gestion de l'enregistrement et dans la lecture du fichier XML. 
	...
	De plus, au fur et à mesure de l'avancement du projet, nous nous sommes rendu compte de quelques erreurs ou incohérence dans le fichier XML. Nous avons donc du corriger ces problèmes, ce qui a engendré des modifications au niveau du code C++ de traitement du fichier XML. 	
...
\end{figure}
	...
	Nous avons bien sur corrigé la DTD associée à ce fichier XML (cf. figure \ref{dtd}). Cette DTD est le modèle, la grammaire, du fichier XML et permet donc de le valider. Elle indique les propriétés de chaque balise ainsi que l'arborescence de notre fichier. 
...
\begin{itemize}
	...
	\item \textbf{NMTOKENS :} indique que l'attribut sera une chaine de caractères réduite (seule une partie des caractères sont autorisés)
	\item \textbf{ENTITY :} indique que l'attribut sera une entité externe qui ne sera pas analysée
\end{itemize}	

\section{Exportation du résultat de l'application mobile}
Pour que l'utilisateur puisse donner le résultat de son travail à l'équipe INNOPHYT, nous avons mis en place un système d'exportation de la base de données vers un fichier au format CSV (avec le poing-virgule comme signe de séparation des colonnes). Cette méthode n'exporte que les données de l'utilisateur qui en fait la demande. Le résultat est enregistré dans le dossier innophyt/export. Les informations exportées sont les suivantes :
\begin{itemize}
\item L'id et le nom de l'utilisateur
\item L'id, le nom, la date de début et de fin, l'adresse, les coordonnées (latitude/longitude) et la description de la campagne
\item Même chose pour les parcelles et les pièges
\item L'id, le nom et le nombre d'insectes
\end{itemize}
...
\begin{itemize}
		\item \textbf{Ajouter un frère :} permets d'ajouter une nouvelle question au même niveau (mais en dernière position) que la question actuellement sélectionnée.  Si l'on se réfère à l'arbre XML, cette question est le frère de la question sélectionnée. Lors du clic, une fenêtre va s'ouvrir permettant de rentrer le texte de la question, ainsi que les critères "visible à l'oeil nu" et "visible à la loupe". 
		\item \textbf{Ajouter un fils :} permets d'ajouter une nouvelle question en tant que fils de la question sélectionnée. Le fonctionnement est identique à "Ajouter un frère". 
		\item \textbf{Modifier :} permets d'afficher une fenêtre, avec ses champs pré-remplis par rapport à la question sélectionnée. 
		\item \textbf{Supprimer :} permets de supprimer une question de la liste. 
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Modifier :} permets de modifier le média sélectionnée. 
	\item \textbf{Supprimer :} permets de supprimer le média sélectionné. 
\end{itemize}
	
	\subsection{Bandeau des réponses}
\begin{itemize}
	\item \textbf{Ajouter réponse :} Permets d'ajouter une nouvelle réponse à la question sélectionnée. Cette réponse est ajoutée à la fin de la liste des réponses. 
	\item \textbf{Modifier réponse :} Permets de modifier le contenu d'une réponse. 
	\item \textbf{Supprimer réponse :} Permets de supprimer une réponse. 
	...
	\item \textbf{Ajouter commentaire :} Permets d'ajouter un média textuel à la réponse sélectionnée. 
	\item \textbf{Modifier média :} Permets de modifier le média sélectionnée. 
	\item \textbf{Supprimer média :} Permets de supprimer le média sélectionné. 
\end{itemize}
...
Pour les coordonnées, nous sommes partis sur le principe suivant (les coordonnées sont entre parenthèse) : 
\begin{verbatim}
Racine (0)
  -- Elem 1 (00)
    -- Elem 11 (000)
...
\\ \\
	Il est ensuite possible de paramétrer son lecteur vidéo avec différents paramètres pour modifier l'aspect de la zone l'affichage comme ajouter un filtre sur l'image. Dans notre cas, nous avons choisi de garder le filtre normal de toute notre fenêtre.
...
\begin{enumerate}
	...
	\item  On ne peut accéder aux éléments définie dans un fichier de layout XML personnalisé que lors du onActivityCreated pas avant, difficilement après.
	...
	\item  Lors de l'ajout de plusieurs fragments à la suite via le code, il faut bien penser qu'un fragment doit obligatoirement être contenu dans un layout.
\end{enumerate}
...
\begin{enumerate}
	...
	\item  Et l'antibanding qui essaye d'atténuer l'effet de bande horizontale, comme lorsque qu'on filme un écran de télé avec un caméscope.
\end{enumerate}
...
Nous avons aussi dû demander au système de réduire la taille des images enregistrées en mémoire d'un facteur de 1/8 (valeur recommandée par plusieurs utilisateurs ayant rencontrés le même problème et confirmer par nos soins) diminuant ainsi grandement les chances de dépassement mémoire.
...
\end{figure}
	...
	Les informations sur les utilisateurs et les projets sont enregistrées dans une base de données SQLite.
...
Pour la dernière table nommée Récolte, le principe de clef étrangère reste le même. Le terme récolte désigne tous les insectes comptés et enregistrés.
\\ \\
...
Ayant voulu exprimer au mieux l'ambiance de l'équipe ainsi que le ressenti de chacun à propos du projet, nous avons décidé de publier un florilège des témoignages de chaque membre de l'équipe. Ceci est plus complet qu'un bilan synthétique écrit par une seule personne tout en restant assez informel et quelque peu personnel. En complément de ces 8 témoignages, vous trouverez aussi les fiches d'auto-évaluation qui ont été remplis personnellement par chacun (fiche manuscrite qui seront remises lors de la soutenance).  
	...
	Pour conclure, je retiendrais une bonne expérience tout de même étant donné que ce projet m'aura permis de voir  comment se passe un projet avec un nombre de personnes assez élevé. \\
Je tiens à féliciter Matthieu qui je trouve a bien géré la situation, ce qui n'est pas évident. Nous avons au final une application fonctionnelle et vraiment agréable. 
	\end{quotation}
	Martin DEMEULEMEESTER --- \textsl{Développeur application PC}.
\\ \\
	\begin{quotation}
	Je ne veux plus toucher de tablette après toutes les difficultés que nous avons rencontrées ^^. Ce fût un vrai challenge et je suis content du résultat que nous avons obtenu. La montée en compétence fût plus longue que je l'espérais, mais reste un bon souvenir maintenant que j'ai une certaine maîtrise.\\
	...
	\end{quotation}
	Julien TERUEL --- \textsl{Développeur application mobile}.
\\ \\	
	\begin{quotation}
...
C'est aussi un projet que je trouvais ambitieux et concret. Avec du recul, je me rends compte que le temps perdu dans certaines parties du développement provient d'un manque de connaissance initiale sur cette technologie.
Mais une fois cette difficulté surmontée et une bonne nuit blanche pour rattraper le retard je trouve le résultat très prometteur. Non seulement il répond aux demandes des clients, mais il peut être une base pour de prochaines évolutions. \\ \\
...
Grâce à ces conditions, nous avons aussi pu réaliser les diverses difficultés d'un projet d'équipe comme les délais à respecter ainsi que d'apprendre de nouvelles technologies indispensables au projet.
	\end{quotation}
	Simon FAUSSIER --- \textsl{Développeur application PC}.
\\ \\	
	\begin{quotation}
	Je suis content d'avoir pu travailler dans cette équipe, aucun conflit n'a été relevé, toutes les personnes sont arrivées au bout de leur taches dans les délais attendu. Je n'ai pas pu m'investir à 100\% dans ce projet pour cause personnelle, mais il a été très intéressant et m'a permis, dans ma partie, d'asseoir mes compétences en C++.
	\end{quotation}
	Jérôme HEISSLER --- \textsl{Développeur application PC}.
\\ \\	
	\begin{quotation}
	...
	\end{quotation}
	Zhengyi LIU --- \textsl{Développeur application mobile}.
\\ \\	
	\begin{quotation}
	...
	\end{quotation}
	Zheng ZHANG --- \textsl{Développeur application mobile}.	
\\ \\	
	\begin{quotation}
...
Comme pour créer un overlay depuis un fichier KML, il faut utiliser GGoXml : map.addOverlay(new GGeoXml('monfichier.kml'));
...
Pour sélectionner une réponse, il vous suffit d'appuyer sur le texte de la réponse ou d'appuyer sur la case à cocher à côté du texte. Soit cela vous mène à d'autres questions, soit une petite fenêtre s'ouvre et vous propose un insecte. Dans ce deuxième cas, s'il correspond, vous pouvez enregistrer votre résultat en indiquant le nombre qu'il y en a dans votre piège.
\\ \\
